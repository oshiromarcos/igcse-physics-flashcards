...
const totals = [...byTopic.values()].reduce(
  ...
  { total: 0, extended: 0, reviewNotes: 0, missingFrontZh: 0, missingBackZh: 0 }
);
...
for (const [topic, stats] of [...byTopic.entries()].sort()) {
  console.log(
    ...
    `${stats.reviewNotes} review notes, ${stats.missingFrontZh} missing frontZh, ` +
    `${stats.missingBackZh} missing backZh`
  );
}
